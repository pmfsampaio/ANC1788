...
uint32_t UDCA[USB_EP_NUM]; // __attribute__ ((section ("usbram")));                     /* UDCA in USB RAM */
uint32_t DD_NISO_Mem[4*DD_NISO_CNT]; // __attribute__ ((section ("usbram")));           /* Non-Iso DMA Descriptor Memory */
uint32_t DD_ISO_Mem [5*DD_ISO_CNT]; // __attribute__ ((section ("usbram"))) ;            /* Iso DMA Descriptor Memory */
...
#if 1
#if 0 //(_CURR_USING_BRD == _IAR_OLIMEX_BOARD)
  LPC_IOCON->P0_27 = 0x2;
  LPC_IOCON->P0_28 = 0x2;
#endif	
...
void USB_Reset (void) {
  ...
  LPC_USB->EpIntEn   = 0xFFFFFFFF; //0x03 | (0x03 << 4);
...
//void USB_IRQHandler (void) __attribute__ ((section (".isrsection")));
